...
void SimpleShadowmapRender::AllocateResources()
{
  ...
  hdr = m_context->createImage(etna::Image::CreateInfo
    {
      .extent = vk::Extent3D{m_width, m_height, 1},
      .name = "hdr",
...
void SimpleShadowmapRender::loadShaders()
{
  ...
  etna::create_program("tone_map", {VK_GRAPHICS_BASIC_ROOT"/resources/shaders/quad3_vert.vert.spv", VK_GRAPHICS_BASIC_ROOT"/resources/shaders/tone_map.frag.spv"});
}
...
void SimpleShadowmapRender::SetupSimplePipeline()
{
  ...
  m_toneMapPipeline = pipelineManager.createGraphicsPipeline("tone_map",
    {
      .fragmentShaderOutput =
      {
        .colorAttachmentFormats = {static_cast<vk::Format>(m_swapchain.GetFormat())}
...
void SimpleShadowmapRender::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkImage a_targetImage, VkImageView a_targetImageView)
{
  ...
  //
  {
    ...
    etna::RenderTargetState renderTargets(a_cmdBuff, {m_width, m_height}, {{hdr.get(), hdr.getView({})}}, mainViewDepth);
  ...
  {
    auto simpleTonemapInfo = etna::get_shader_program("tone_map");

    auto set = etna::create_descriptor_set(simpleTonemapInfo.getDescriptorLayoutId(0), a_cmdBuff,
      {
        etna::Binding {0, constants.genBinding()},
        etna::Binding {1, hdr.genBinding(defaultSampler.get(), vk::ImageLayout::eShaderReadOnlyOptimal)}
      });
